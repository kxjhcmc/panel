...
const { data: config, send: refreshConfig } = useRequest(valkey.config, {
  initialData: '',
})
...
const { data: load } = useRequest(valkey.load, {
  initialData: [],
})
...
const handleSaveConfig = () => {
  ...
  useRequest(valkey.saveConfig(config.value))
    .onSuccess(() => {
      ...
      <n-tab-pane name="config" :tab="$gettext('Main Configuration')">
        <n-flex vertical>
          <n-alert type="warning">
            {{
              $gettext(
                'This modifies the Valkey main configuration file. If you do not understand the meaning of each parameter, please do not modify it randomly!',
              )
            }}
          </n-alert>
