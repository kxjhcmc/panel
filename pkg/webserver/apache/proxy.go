...
func generateProxyConfig(proxy types.Proxy) string {
	...
	_, _ = fmt.Fprintf(&sb, "    ProxyPass %s %s\n", location, proxy.Pass)
	_, _ = fmt.Fprintf(&sb, "    ProxyPassReverse %s %s\n", location, proxy.Pass)
	...
	if proxy.Cache != nil {
		...
		_, _ = fmt.Fprintf(&sb, "        CacheEnable disk %s\n", location)
		...
		_, _ = fmt.Fprintf(&sb, "        CacheDefaultExpire %d\n", expireSeconds)
	...
	if len(proxy.Headers) > 0 {
		...
		for name, value := range proxy.Headers {
			_, _ = fmt.Fprintf(&sb, "        RequestHeader set %s \"%s\"\n", name, value)
		}
		sb.WriteString("    </IfModule>\n")
	}
	...
	if len(proxy.Replaces) > 0 {
		...
		for from, to := range proxy.Replaces {
			// 使用 | 作为分隔符以支持包含 / 的内容
			_, _ = fmt.Fprintf(&sb, "        Substitute \"s|%s|%s|n\"\n", from, to)
		}
		sb.WriteString("    </IfModule>\n")
	}
